			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - Kolam.id</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - Kolam.id</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="https://helpful-bear-2de3f8.instawp.site/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
