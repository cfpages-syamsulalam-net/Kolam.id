...
function pageGeneratorProGutenbergRegisterBlock( block ) {
	...
	( function ( blocks, editor, element, components, block ) {
		...
		if ( typeof block.gutenberg_icon !== 'undefined' && block.gutenberg_icon !== false ) {
						...
						for ( var i in block.tabs[ panel ].fields ) {
							...
							switch ( field.type ) {
								...
								case 'autocomplete_textarea':
									// There's no autocompleter for a textarea in Gutenberg using datalist.
									// Just return a TextareaControl.
									fieldElement = el(
										TextareaControl,
										fieldProperties
									);
